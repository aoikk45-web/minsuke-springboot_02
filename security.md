...
| イベント担当講師の設定 | **CU** | R | — |
| 講師稼働（担当イベント） | R | R | — |
| お知らせ作成・編集・削除 | **CRUD** | R（既読可） | — |
| スケジュール CRUD・イベント生成 | **CRUD + 生成** | — | — |
| ユーザー管理 | CRUD | — | — |

### Loop 08 認可方針（Confirmed）
...
### Loop 10 認可方針（Proposed）

| 操作 | ロール | 備考 |
|---|---|---|
| お知らせ一覧・詳細 | 認証済み全員 | 詳細表示で既読記録 |
...
| 操作 | ロール | 備考 |
|---|---|---|
| 参加登録単位の設定 | **ADMIN のみ** | スケジュール／イベント作成・編集 |
...
- ログイン試行回数制限・ロックアウト
- 個人情報保護法上の利用目的・同意取得
- 本名・連絡先・メールを MinSuke に置かず外部会員へ寄せるか（**OQ-P01〜P05** — Loop 18 Approved。`minutes.md` §35）
- ログイン識別子は **`login_id`（アカウント ID）**（OQ-P04）
- 本番 HTTPS 証明書・運用
- セキュリティテストの自動化範囲
