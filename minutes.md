...
| 作業 | 状態 |
|---|---|
| 設計草案（汎用レポート＋年齢・性別＋チャネル方針） | ✅ **2026-08-23**（`minutes.md` §36）— **方針変更により中止** |
| 人間承認 | ❌ **中止**（CTX-02） |
| 実装 | しない |
...
| OQ-06 | 同時利用規模 | Open（クラブ 1 万人前提は CTX-02 で撤回） |
| OQ-07 | 本番デプロイ | Open |
...
| OQ-A01〜A09 | クラブ報告・外部属性・教室 | ❌ **Withdrawn**（CTX-02。Loop 19 中止） |
| OQ-CS01 | サークルスクエア参考の次機能 | **Parked**（§38。候補 A〜D。未承認。Loop 20 の対象外） |
| OQ-T01 | CI で何を回すか | ✅ **Approved** Loop 20 — `mvnw test` + Testcontainers（GHA） |
...
## CTX-02 — プロダクト位置づけの再定義

- **Date:** 2026-08-23
- **Decision:** MinSuke は **地域のスケジュール管理アプリ**（カレンダー・イベント・参加登録・当番の見える化）。総合型地域スポーツクラブの会員マスタ・決済・補助金報告は **外部一体型** に任せる。
- **Impact:** Loop 19 中止。OQ-A01〜A09 withdrawn。Loop 18 の login_id / 家庭サブスク参照は残す（コードを戻さない）。クラブ向け属性レポートは作らない。
- **Approved:** Yes

## Loop 20 — Testing / CI

- **Date:** 2026-08-23
- **Decision:** 機能追加なし。GitHub Actions で `./mvnw test`（Testcontainers）。`testing.md` を正式化。E2E・カバレッジ失敗ゲートは含めない。
- **Approved:** Yes（人間「Testing/CI に進み、一旦完成」）
...
# 36. Loop 19 — 管理者向け汎用活動レポート（設計草案 2026-08-23）

> **中止（CTX-02）:** 本節は参考。実装しない。決定は §37。

人間がコミュニティースポーツスクールの報告用データ集計を希望。年齢・性別が必要。続けて全体像を提示（2026-08-23）:

- **公式 LINE をベース**に、Homepage / Instagram へ。
- **スケジュール・参加登録**はアプリ（MinSuke）へ。
- **個人情報と決済**は外部へ。
- 報告は **汎用**（特定の補助金様式に固定しない）。
- 想定利用者 **約 1 万人**。

人間追記（2026-08-23）: **外部 ID は一人ずつ。** 集計時に外部から年齢・性別を取得。MinSuke にはできるだけ個人情報を持たせない。
...
公式 LINE（入口・導線）
  ├ Homepage（紹介）
  ├ Instagram（発信）
  ├ MinSuke（カレンダー・参加登録・お知らせ）
  └ 外部会員＋決済（本名・連絡先・課金）
```

MinSuke の役割は **予定と参加のシステム・オブ・レコード**。発信と会員マスタは外に置く（Loop 18 と一致）。

## 目的（Loop 19）

ADMIN が **任意の期間** で活動実績を集計し、CSV で持ち帰れる。補助金・クラブ内報告・年度まとめなど、用途をコードに焼き込まない。

## 個人情報かどうか（人間質問 2026-08-23）

**生年月日も性別も個人情報にあたる。** 個人情報保護法上、生存する個人を識別できる情報に含まれる。表示名と組み合わせればなおさら識別しやすい。

Loop 18 で外したのは **連絡先と決済**（本名・住所・電話・メール・カード）であり、「MinSuke に個人情報を一切置かない」ではない。ログイン ID・表示名・参加履歴もすでに個人情報になり得る。

| 層 | 例 | MinSuke |
|---|---|---|
| 連絡先・決済 | 本名、住所、電話、メール、カード | **置かない**（外部） |
| 統計用属性 | 生年月日、性別 | **外部が正。MinSuke はコピーしない**（OQ-A08） |
| 運用 | 表示名、login_id、参加、外部会員 ID | 置く |

取り扱いの方針（案）:

- 利用目的を「活動実績の統計・報告」に限定する
- カレンダーや参加画面に性別・生年月日を出さない
- 既定の CSV は **集計表**。個人行に生年月日を載せない
- 集計時だけ外部 API から属性を読み、DB に残さない（またはごく短いメモリ上）

## 外部から引っ張る（OQ-A08・人間質問 2026-08-23）

**できる。目標アーキテクチャとしてはそれが正しい。** 会員登録の正は外部なので、生年月日・性別もそこで取り、報告時に MinSuke が参照する。

いまの制約:

- 外部会員システムは未接続（`BillingPort` / メール Port は Stub）
- 外部 ID は **家庭**（`households.external_member_id`）だけ。保護者・子ども一人ずつの ID がない
- 1 万人の報告で 1 人ずつ HTTP すると遅い。**一括取得**（ID リスト → 生年月日・性別）が必要

| 案 | 内容 | 評価 |
|---|---|---|
| **A08-A. 外部が正（推奨）** | `parents` / `children` に `external_member_id`。`MemberProfilePort.lookupDemographics(ids)`。MinSuke に生年月日・性別カラムを **作らない** | Loop 18 と一致。外部が来るまでクロス表は「不明」 |
| A08-B. MinSuke にコピー | フォームで生年月日・性別を入力 | 外部と二重管理。個人情報が増える |
| A08-C. 両方 | 外部優先、なければローカル | 最悪の二重保持 |

**Loop 19 の推奨:** A08-A。人単位の `external_member_id`（保護者・子ども）。`MemberProfilePort.lookupDemographics(ids)` 一括。MinSuke に生年月日・性別カラムなし。Stub 中はクロス表「不明」。ベンダーは Port の向こうに隠す（OQ-A09）。

## 適合しそうな外部システム（調査 2026-08-23・未選定）

MinSuke が求める契約: **人ごとの ID**、生年月日・性別、**ID リストの一括取得**、できれば家庭グルーピングと課金状態。LINE 公式は入口向きだが、生年月日の正本には向かない。

| 種類 | 例 | 向き | 注意 |
|---|---|---|---|
| 運動施設向け会員＋決済 | [hacomono](https://www.hacomono.jp/)、両備 [ATOMS for U](https://service.ryobi.co.jp/business_innovation/atoms-for-u/)、スコラプラス | 教室・月謝・会員属性が近い | 予約・出欠まで持つと MinSuke と機能が重なる。API・一括取得の有無を契約前に確認 |
| 会員マスタ（自前スキーマ）＋決済分離 | **kintone**（人レコード＋世帯）＋ Stripe / Pay.jp（家庭サブスク） | PII を外に置き、MinSuke は ID だけ。1 万人・一括 API 向き。ベンダーロックが相対的に弱い | 会員画面・帳票は自分で組む |
| CRM 大型 | Salesforce | 一括 API・権限は強い | コスト・運用が過大になりやすい |
| 決済のみ | Stripe / PAY.JP の Customer | メール・請求住所は取れる。**家庭の課金**向き | **生年月日・性別は標準項目にない。** metadata に載せられるが会員マスタ代わりには向かない。子ども一人ずつ・一括取得にも不向き |
| 公式 LINE | LINE Login / ミニアプリ | 入口・通知 | プロフィールに生年月日・性別の正本を置かない |

**選定の目安（まだベンダーは決めない）:** REST（または同等）で人 ID の一括 GET ができること、家族（世帯）でまとめられること、日本での個人情報の取扱いが説明できること。運動施設オールインワンを入れるなら、予定・参加は MinSuke に残し、会員マスタと決済だけ使う範囲を契約で切る。

初期の実装はベンダー名をコードに書かず `MemberProfilePort` のみ。

## 年齢・性別の置き方

正本は外部。MinSuke は開催日時点の満年齢を計算して帯に入れる（データは応答の中だけ）。

| 対象 | 集計の切り口 |
|---|---|
| **子ども** | 学校相当の年齢帯（未就学 / 小 / 中 / 高） |
| **保護者（大人）** | **年代**（10代 / 20代 / … / 70代以上） |
| 取れない参加 | 「不明」 |

## 教室ごとの集計（OQ-A07）

MinSuke に「教室」マスタはない。コミュニティースポーツの「教室」は、既存の **スケジュール**（例: 旗当番、定期クラス）に対応させる。

| 切る単位 | 中身 |
|---|---|
| **教室** | `schedules`（タイトル単位）。期間内のそのスケジュールから生成されたイベントの参加を集計 |
| 単発 | `schedule_id` なしのイベントは「単発・その他」 |

班（`households.group_name`）は教室ではない。会場マスタが必要なら後続。
...
| 操作 | 内容 |
|---|---|
| 期間 | 開始日〜終了日（プリセット: 今年度 / 先月 / 全期間） |
| 教室 | 全教室 / 1 スケジュール |
| サマリー | 開催回数、延べ参加、実人数（子ども・保護者） |
| クロス（子ども） | 年齢帯 × 性別 |
| クロス（大人） | **年代 × 性別** |
| 明細 | 日付・教室（スケジュール）・タイトル・講師・定員・登録数 |
| 出力 | 集計 CSV（ADMIN のみ）。個人の生年月日は出さない |

## 規模（OQ-06）

| 指標 | 想定 | Loop 19 での扱い |
|---|---|---|
| 登録利用者 | **約 1 万人** | 目標規模として記録。単一 Spring Boot + PostgreSQL で十分なレンジ |
| 同時接続 | 未提示 | ピーク設計は後続（NFR-07） |
| Loop 19 | 集計は期間フィルタ＋SQL 集約。一覧はページネーションまたは CSV | 全件 HTML は避ける |

## 候補

| 案 | 内容 | 評価 |
|---|---|---|
| **A. 汎用レポート＋外部属性 Port（推奨）** | 期間・教室・子ども年齢帯／大人年代×性別、CSV。属性は `MemberProfilePort`。生年月日カラムは作らない | 個人情報を増やさない。Stub 中は「不明」 |
| B. MinSuke に生年月日・性別を入力 | 外部未接続でも数字が出る | 連絡先の次に属性 PII が増える |
| C. LINE 公式＋レポート一括 | Messaging API・LIFF | 本 Loop では過大 |

## OQ 推奨回答（承認対象）

| ID | 質問 | 推奨 |
|---|---|---|
| **OQ-A01** | 年齢の計算 | 外部の **生年月日** から、開催日時点の帯へ。MinSuke には保存しない |
| **OQ-A02** | 性別 | 外部の値を `FEMALE` / `MALE` / `UNSPECIFIED` に写像（メモリ上） |
| **OQ-A03** | 対象 | 子どもも保護者も、**人単位の外部 ID** で引く |
| **OQ-A04** | 基準日 | **各イベント開催日**。大人は **年代** |
| **OQ-A05** | レポート形 | **汎用**（期間・教室・集計 CSV） |
| **OQ-A06** | LINE / HP / Instagram | 導線方針のみ。実装は後続 |
| **OQ-A07** | 教室 | **スケジュール単位**。単発は「その他」 |
| **OQ-A08** | 属性の置き場 | **外部が正。人単位 ID。Port 一括取得。MinSuke に生年月日・性別カラムを作らない**（人間確認 2026-08-23） |
| **OQ-A09** | 外部プロダクト | **未選定。** Port で抽象化。候補は hacomono / ATOMS / スコラプラス（重複注意）、または kintone＋決済 |
| **OQ-06** | 規模 | 登録 **約 1 万人**。Port は一括 API |

## 含まない（後続）

LINE 公式アカウント連携、Homepage / Instagram のホスティング、グラフ、自治体 PDF、Testing/CI、本番同時接続設計、**MinSuke への生年月日・性別の複製**。

## 参照

- `minutes.md` §35（PII 境界）
- `requirements.md` FR-E07 / NFR-06 / OQ-06
- `database.md` children.birth_date

人間が「MinSuke を使わず外部だけにした方がよいのでは」と相談（2026-08-23）。続きは §37。

---

# 37. 戦略 — 自前 MinSuke vs 外部一体型（Approved 2026-08-23）

要件（LINE 入口、予定・参加、PII・決済外出し、人単位属性、教室別集計、1 万人）は、hacomono / ATOMS / スコラプラス等の **会員＋予約＋決済一体型** と大きく重なる。

| 取る道 | 向くとき | 代償 |
|---|---|---|
| **外部一体型だけ** | 運用を早く始めたい。予定も決済も一つの画面でよい。旗当番の公平など独自要件が弱い | ベンダー依存。MinSuke の参加単位・当番率などは無い／要カスタム。データ移行が後で重い |
| **MinSuke を核** | 参加の切り方（家庭／保護者／子ども）、当番の公平、補助金向けの **汎用集計** を自分で持ちたい | 会員マスタ＋決済＋LINE と **3 系統**。連携と ID 設計が仕事になる |
| **ハイブリッド** | 予定・参加は MinSuke、PII・月謝は外部（現行方針） | 一番柔軟だが、一番つなぎが多い |

Loop 19 はハイブリッド前提のレポート。一体型に寄せるなら **実装せず製品比較（API・出欠・帳票・LINE）を先に** する。

## 決定（Approved 2026-08-23）

人間が方針変更。**総合型地域スポーツクラブは外部システム。MinSuke は地域のスケジュール管理アプリに戻す（CTX-02）。**

| MinSuke に残す | 外部（クラブ運営） |
|---|---|
| 月間カレンダー、イベント、参加登録・キャンセル、定員 | 会員の本名・住所・メール・生年月日・性別 |
| 家庭の表示名、講師、スケジュール（当番・定期） | 月謝・決済 |
| 参加履歴、ADMIN の家庭参加率（Loop 16〜17） | 補助金・教室別の公的報告 |
| アプリ内お知らせ | 公式 LINE / HP / Instagram の本営 |

Loop 19 は実装しない。§36 は参考として残し、OQ-A は withdrawn。

---

# 38. サークルスクエア比較（記録 2026-08-23）

人間依頼。公式機能一覧（https://www.c-sqr.net/pub/funclist）と MinSuke 現状を比較。**この時点で実装しない。次 Loop は人間承認待ち。**

サークルスクエアは団体運営グループウェア（出欠・チャット・名簿・会計・HP など約 18 機能）。MinSuke は CTX-02 どおり **地域のスケジュール・参加登録**。全部は真似しない。

## すでに近いもの

| サークルスクエア | MinSuke |
|---|---|
| カレンダー／出欠／定員 | 月間カレンダー、個別参加、定員 |
| 定期予定 | スケジュール（週次・単発）＋生成イベント |
| インフォメーション＋未読 | お知らせ＋既読 |
| 名簿・連絡先 | 表示名中心。連絡先・決済は外（Loop 18） |

MinSuke の強み（薄めない）: 家庭単位・保護者／子ども単位の参加、講師・当番、家庭参加率。サークルスクエアはメンバー個人中心。

## 入れた方がよい（スケジュール管理として効く）

1. **場所・集合時間** — 開始と別の集合時刻、場所（マスタ＋地図は後回し可）
2. **キャンセル待ち** — 定員の延長
3. **予定変更・開始前リマインド** — まずはアプリ内。「この予定が変わった」「明日ある」
4. **出欠の粒度（任意）** — 遅刻・早退・コメントはイベント単位のオプション
5. **招待リンク／QR** — 運営がリンクを配って家庭を足す
6. **週間ビュー** — 当番・週次確認

## 直した方がよい（既存の磨き）

- 予定詳細が薄い（集合／場所／持参物／服装）
- 日時変更が参加済み家庭に伝わらない（お知らせとイベントが未接続）
- 満員後の待ち・繰り上げがない
- お知らせ対象が「認証済み全員」固定（「このスケジュールの参加家庭」があると自然）
- 定期が週次中心（毎月第 n 曜日は未実装）
- 小規模団体向けにナビを薄くする選択肢

## 入れない方がよい

名簿の本名・連絡先・CSV、会計、会費、チャット／つぶやき／掲示板／ブログ、公開 HP、アルバム、フォルダ、グループごとの機能制限、メール一括配信。アンケート・日程調整は出欠で足りるなら後回し。

## 次 Loop 候補（未承認）

| 案 | 内容 |
|---|---|
| **A（推奨）** | イベント／スケジュールに場所・集合・補足。変更時は参加家庭向けアプリ内通知 |
| **B** | キャンセル待ち |
| **C** | 招待リンクで家庭追加 |
| **D** | カレンダー週間ビュー |

**Approved:** 記録のみ。候補の採否は次 Loop の人間承認。

---

# 39. Loop 20 — Testing / CI（Approved 2026-08-23）

人間が Testing/CI でアプリを一旦完成させると判断。サークルスクエア候補（§38）は対象外。

| 含む | 含まない |
|---|---|
| `.github/workflows/ci.yml`（JDK 21、`./mvnw -B test`） | 新画面・新機能 |
| `testing.md` | Selenium / Playwright E2E |
...
| 観点 | 証拠 |
|---|---|
| A. 文書 | Loop 20 = `minutes` / `Composer` §4.18 / `roles` / `roadmap` / `README` / `testing.md` |
| B. DB | スキーマ変更なし。テストは Flyway 本番 migrations（seed なし） |
| C. Security | 画面・認可の変更なし。`EventServiceTest.inactiveSubscriptionPreventsRegistration` で OQ-P05 を固定 |
| D. 環境 | CI: `.github/workflows/ci.yml` JDK 21。ローカル Testcontainers は Docker Engine 29 向け `docker-java.properties` |
| E. 画面 | 変更なし（確認不要） |
| F. テスト | `mvnw test` **96** 件、Failures 0、Skipped 0（2026-08-23 ローカル） |
...
- **Status:** **CANCELLED**（CTX-02 — クラブ報告は外部。MinSuke はスケジュール管理）
- **Started:** 2026-08-23
- **Cancelled:** 2026-08-23
- **Branch:** 草案のみ（実装ブランチなし）
- **Last Updated:** 2026-08-23
- **Next Action:** —
...
- **Branch:** `feature/loop-17-admin-participation`（merged to main via PR **#13**）
- **Last Updated:** 2026-08-17 — main へ merge 済
- **Next Action:** —
