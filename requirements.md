...
| FR-I05 | イベントへの担当講師の設定 | Proposed | **No（Loop 09）** | OQ-03 確定後。単一 or 複数は設計時に決定 |
| FR-I06 | 講師の稼働状況の可視化 | Proposed | **No（Loop 09 以降）** | 担当件数・期間・空き状況など。詳細は Loop 09 設計 |
...
| ID | 要件 | 区分 | MVP |
|---|---|---|---|
| FR-N01 | お知らせ作成・配信 | Proposed | No |
| FR-N02 | 配信対象の指定 | Proposed | No |
| FR-N03 | 既読管理 | Proposed | No |
...
| OQ-03 | スケジュールとイベントの関係 | Open — **Loop 09 前に確定**（担当講師・稼働の置き場所に影響） | データモデル |
