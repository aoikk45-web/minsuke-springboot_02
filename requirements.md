...
### Confirmed（CTX-02 / 2026-08-23）

- MinSuke は **地域のスケジュール管理アプリ**（カレンダー、イベント、参加登録、当番の見える化）
- 総合型地域スポーツクラブの **会員マスタ・決済・補助金報告は外部一体型** に任せる（本プロダクトの範囲外）
...
| FR-E05 | イベント履歴・参加一覧 | **Approved（Loop 16・最小）** | No |
| FR-E06 | 自家庭が参加しているイベントの識別（カレンダー色・本日参加） | **Approved（Loop 13）** | No |
| FR-E07 | 管理者の参加状況・家庭参加率 | **Approved（Loop 17・最小）** | No |

#### FR-E06 詳細（Loop 13 Approved — 2026-08-14）

保護者が「自分の家族がどのイベントに出るか」をカレンダー上で把握できるようにする。

| 案 | 内容 | 評価 |
|---|---|---|
| **A+B（推奨）** | カレンダーで参加中イベントの色を変える + 本日の参加イベントを一覧表示 | 既存 S03 で完結。新テーブル不要 |
| A のみ | 色だけ | 一覧がないと「今日何があるか」が一覧しづらい |
| B のみ | 本日一覧だけ | 月内の他日の参加が見えない |
| C | 参加履歴専用ページ（FR-E05） | カレンダーと二重。後続でよい |

- **参加の定義:** 自家庭の `event_attendances` が `REGISTERED` なら 1 件でも「参加中」（HOUSEHOLD / PARENT / CHILD いずれも）
- **他家庭の参加は出さない**（自分の household_id のみ）
- **ADMIN**（household なし）: 色分け・本日参加は出さない（現状どおり全イベント表示）
- 満員表示は維持。参加中かつ満員は参加色 + 「満」
- FR-E05（履歴ページ）とは別。本機能はホーム（S03）の視認性

#### FR-E05 詳細（Loop 16 Approved — 2026-08-15）

PARENT が自家庭の参加中イベントを、カレンダーの月をまたいで一覧できる。

| 項目 | 内容 |
|---|---|
| 画面 | `GET /my-participations`（S24）。ヘッダー「参加履歴」（PARENT） |
| レコード | 自 household の `event_attendances.status = REGISTERED` のみ |
| 期間 | 今後＋過去。開催日降順（同日は開始時刻・イベント ID 降順） |
| 列 | 日付・時間・イベント名（詳細リンク）・参加者名・区分（保護者/子ども/家庭） |
| 認可 | PARENT のみ。ADMIN は 403。他家庭は出さない |
| 含まない | CANCELLED、CSV、ADMIN 全家庭レポート、FR-F06、グラフ、ページネーション |

#### FR-E07 詳細（Loop 17 Approved — 2026-08-17）

ADMIN がシリーズ（旗当番など）の **各家庭の参加率** と、月次のイベント充足を見る。

| 項目 | 内容 |
|---|---|
| 主 | `GET /schedules/{id}/participations` — 家庭名・参加回数・対象回数・参加率（低い順）。全家庭（0% 含む） |
| 副 | `GET /admin/participations` — 対象月の定員・登録数・空き |
| 参加 | その回に household の REGISTERED が1件以上なら 1 回 |
| 認可 | ADMIN のみ |
| 含まない | CSV、グラフ、PARENT、CANCELLED 分析、FR-F06 |

### OQ-E01 参加可視化の出し方（Loop 13 Approved）

**状態:** ✅ **Approved**（2026-08-14）。A+B（カレンダー色 + 本日一覧）を採用。
...
### OQ-S02 参加登録単位（Loop 12 Approved）
...
**状態:** ✅ **Approved**（2026-08-14）。A+B を採用。
...
| P2 | **FR-E06 自家庭の参加可視化**（カレンダー色・本日参加） |
| P3 | 一括登録・インポート |
| P3 | 参加履歴・レポート（FR-E05） |
| P3 | 管理者の参加状況・家庭参加率（FR-E07 — Loop 17 草案） |
| P3 | Spring Security 相当の本格認可 |
| P4 | 外部通知（メール・プッシュ） |
| P4 | 個人情報・決済の外部化（ニックネーム運用、家庭サブスク、お知らせメール）— **OQ-P01〜P05** |
...
- 外部サービス連携（メール、認証プロバイダ等）→ **OQ-P01〜P05**（`minutes.md` §35 / Loop 18 設計中）
- 本番運用・課金の有無 → 家庭サブスクは外部決済（OQ-P02）
...
| OQ-S02 | スケジュールごとの参加登録単位（家庭/保護者/子ども） | ✅ **Approved** — schedule + event（FR-S03） | DB・UI・EventService |
| OQ-E01 | 自家庭の参加をカレンダーでどう見せるか | ✅ **Approved** — 色 + 本日一覧（FR-E06） | カレンダー UI |
| OQ-M01 | Loop 14 の対象 | ✅ **Approved** — Mobile UI | UI |
| OQ-M02 | カレンダーの狭い画面 | ✅ **Approved** — グリッド維持 + セル縮小 | カレンダー UI |
| OQ-M03 | ナビ | ✅ **Approved** — ハンバーガー | ヘッダー |
| OQ-04 | 子どもの直接ログイン要否 | Open | 認証・UI |
| OQ-05 | 個人情報の保持期間・削除ポリシー | ✅ **選択肢 B（標準保持）** | セキュリティ・DB |
| OQ-06 | 同時利用規模・可用性要件 | Open（クラブ 1 万人前提は CTX-02 で撤回） | インフラ |
...
| OQ-P01 | 本名・連絡先を MinSuke に持たずニックネーム運用＋外部会員か | ✅ **Approved** Loop 18（表示名中心・phone 削除） | 家族・認証・個人情報 |
| OQ-P02 | 登録家庭のサブスク決済を外部で行うか | ✅ **Approved** Loop 18（status 参照のみ・単位 household） | 課金・household |
| OQ-P03 | お知らせメールを外部の登録 email から送るか | ✅ **Approved** Loop 18（SMTP なし・Port Stub） | 通知 |
| OQ-P04 | ログインを外部 SSO にするか、当面 email のままか | ✅ **Approved** Loop 18（**login_id + password**） | 認証 |
| OQ-P05 | 未払い時に MinSuke で何を止めるか | ✅ **Approved** Loop 18（参加登録のみ停止） | 認可 |
| OQ-R01 | Loop 17 の対象 | ✅ **Approved** — シリーズ家庭参加率＋月次充足 | 管理画面 |
| OQ-R02 | 家庭の1回参加の定義 | ✅ **Approved** — その回の REGISTERED が1件以上 | 集計 |
| OQ-R03 | 参加率の分母 | ✅ **Approved** — 期間内の生成済みイベント数 | 集計 |
| OQ-R04 | 出す家庭 | ✅ **Approved** — 全家庭（0% 含む） | 集計 |
| OQ-R05 | 期間の初期値 | ✅ **Approved** — 生成済み全期間 | UI |
| OQ-R06 | 月次充足表を含めるか | ✅ **Approved** — 含める | UI |
| OQ-A01 | 参加者の年齢をどう持つか | ❌ **Withdrawn**（CTX-02。クラブ報告は外部） | — |
| OQ-A02 | 性別の持ち方 | ❌ **Withdrawn** | — |
| OQ-A03 | 子ども以外にも性別・生年月日を持たせるか | ❌ **Withdrawn** | — |
| OQ-A04 | 年齢の基準日 | ❌ **Withdrawn** | — |
| OQ-A05 | レポート形 | ❌ **Withdrawn** | — |
| OQ-A06 | LINE / HP / Instagram | ❌ **Withdrawn**（チャネル本営はクラブ外部） | — |
| OQ-A07 | 教室の単位 | ❌ **Withdrawn** | — |
| OQ-A08 | 生年月日・性別を MinSuke に置くか外部か | ❌ **Withdrawn**（クラブ CRM は外部。MinSuke に属性を持たない） | — |
| OQ-A09 | 外部会員システムの製品 | Open（クラブ運営側の選定。MinSuke 範囲外） | — |
