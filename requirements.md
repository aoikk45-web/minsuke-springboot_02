...
- 外部サービス連携（メール、認証プロバイダ等）→ **OQ-P01〜P05**（`minutes.md` §33。後続 Loop）
- 本番運用・課金の有無 → 家庭サブスクは外部決済を候補（OQ-P02）
...
| OQ-P01 | 本名・連絡先を MinSuke に持たずニックネーム運用＋外部会員か | Open（後続 Loop） | 家族・認証・個人情報 |
| OQ-P02 | 登録家庭のサブスク決済を外部で行うか | Open（後続 Loop） | 課金・household |
| OQ-P03 | お知らせメールを外部の登録 email から送るか | Open（後続 Loop。OQ-08 将来分） | 通知 |
| OQ-P04 | ログインを外部 SSO にするか、当面 email のままか | Open（後続 Loop） | 認証 |
| OQ-P05 | 未払い時に MinSuke で何を止めるか | Open（後続 Loop） | 認可 |
